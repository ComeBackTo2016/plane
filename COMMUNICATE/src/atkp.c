...
static void atkpSendPeriod(void)
{
	...
	{
		int baro = getBaroData() * 1.f;
		Axis3f test_pos;
		hyfgetStateDataAcc(&test_pos);
//		hyfgetEstimator_velpos(&test_pos);
//		hyfgetStateDataPos(&test_pos);
//		int fusebaro = getFusedHeight() * 100.f;
//		sendSenser2(baro, (int)test_pos.z);
		sendSenser2((int)(test_pos.z), (int)(test_pos.y));
	}
	if(++count_ms>=65535) 
		count_ms = 1;	
}
